...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        ouliers_removed_df.loc[ouliers_removed_df["label"] == label, col] = dataset[col]

        n_ouliers = len(dataset) - len(dataset[col].dropna())
        print(f"Removed {n_ouliers} from {col} for { label}")


ouliers_removed_df.info()
# --------------------------------------------------------------
# Export new dataframe
# --------------------------------------------------------------

ouliers_removed_df.to_pickle("../../data/interim/02_outliers_removed_chauvenets.pkl")
